...
class ClockifyUtils:
    ...
    def get_workspaces(self, settings: dict = None):
        ...
        workspaces = make_call_with_retry(
            "get", workspace_url, info="fetch clockify workspaces for selection"
        )
        workspace_list = {}
        for ws in workspaces:
            workspace_list[ws["name"]] = ws["id"]

        workspace_id = workspace_list[
            list_options(
                workspaces,
    ...
    def get_tasks_by_project(self, settings, project: str):
        ...
        task_list = {
            task["name"]: task["id"] for task in tasks if task["status"] != "DONE"
        }
        done_list = {
            task["name"]: task["id"] for task in tasks if task["status"] == "DONE"
        }
