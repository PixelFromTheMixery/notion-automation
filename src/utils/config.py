...
class Config:
    ...
    def setup(self, settings: dict, services: list, databases=None):
        if "mover" in services:
            ...
            settings["notion"]["source"] = databases[
                list_options(
                    notion_dbs,
            ...
            settings["notion"]["destination"] = databases[
                list_options(
                    notion_dbs,
                    "Enter the number of the database: ",
            ...
            # Notion Settings
            prop_name = input(
                "Enter the name of the property to check for completion: "
            )
            settings["notion"]["prop_name"] = prop_name
            checkbox = bool(
                input("Does this property use a checkbox? (1 for yes, 0 for no): ")
            )
            if checkbox:
                settings["notion"]["checkbox"] = False
                reset_text = input(
                    "Enter the name of the option you'd like to reset the status to: "
                )
